...
void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : MAG_DRDY_Pin TF_CD_Pin */
  GPIO_InitStruct.Pin = MAG_DRDY_Pin|TF_CD_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

  /* TF_CD 单独上拉：卡座开关"有卡接地"，若板上无外部上拉，无卡时该脚悬空会误读。
     启用内部上拉后无卡稳定读 1、有卡读 0，test_sd() 才能区分无卡与焊点不良。 */
  GPIO_InitStruct.Pin = TF_CD_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_PULLUP;
  HAL_GPIO_Init(TF_CD_GPIO_Port, &GPIO_InitStruct);

  /*Configure GPIO pins : IMU_CS_Pin BUZZER_Pin */
  GPIO_InitStruct.Pin = IMU_CS_Pin|BUZZER_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
